...
func (r *RestaurantRepo) CreateMenu(mc *types.MenuCreate) error {
	...
	for _, v := range slp {
		var id, t int
		err := r.DB.QueryRow(context.Background(), "SELECT uuid, type_id  FROM product WHERE name = $1", v).Scan(&id, &t)
		if err != nil {
			fmt.Errorf("Error to get id and type from db: %s", err)
		}
		err = r.DB.QueryRow(context.Background(), "INSERT INTO menu (on_date, opening_record_at, closing_record_at, product_id, prod_type_m) VALUES ($1, $2, $3, $4, $5) RETURNING uuid",
			mc.OnDate,
			mc.OpenAt,
			mc.ClosedAt,
			id,
